#ifndef GPU_PORTABLE_GL_DEVICE_H
#define GPU_PORTABLE_GL_DEVICE_H
...
namespace gpu
{
  ...
  class PortableGLDevice final : public Device
  {
  public:
    explicit PortableGLDevice(const DeviceDesc &desc);
    ~PortableGLDevice() override;
  ...
  private:
    ...
    char *nextDiagnostic();
    static void GLAD_API_PTR debugMessage(GLenum source, GLenum type,
  ...
  Device *createOpenGLDevice(const DeviceDesc &desc);
