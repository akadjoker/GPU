#include "PortableGLDevice.h"

#if !defined(GPU_GL_DESKTOP)
#define GPU_GL_DESKTOP 1
#endif
...
bool compressedTextureFormat(Format format, CompressedFormatInfo &info) {
  switch (format) {
  case Format::BC1RGBA:
    info = {GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, 8};
    return true;
  case Format::BC1RGBASrgb:
    info = {GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT, 8};
    return true;
  case Format::BC3RGBA:
    info = {GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, 16};
    return true;
  case Format::BC3RGBASrgb:
    info = {GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT, 16};
    return true;
  case Format::BC5RG:
    info = {GL_COMPRESSED_RG_RGTC2, 16};
    return true;
  case Format::BC7RGBA:
    info = {GL_COMPRESSED_RGBA_BPTC_UNORM, 16};
    return true;
  case Format::BC7RGBASrgb:
    info = {GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM, 16};
    return true;
  ...
  case Format::ASTC4x4RGBA:
    info = {GL_COMPRESSED_RGBA_ASTC_4x4_KHR, 16};
    return true;
  case Format::ASTC4x4RGBASrgb:
    info = {GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR, 16};
    return true;
  default:
    return false;
  }
...
GLADapiproc loadGLProcedure(void *userData, const char *name) {
  const GLSurface *surface = static_cast<const GLSurface *>(userData);
  return reinterpret_cast<GLADapiproc>(
      surface->getProcAddress(surface->userData, name));
}
...
Device *createOpenGLDevice(const DeviceDesc &desc, GPUError *error) {
  if (desc.backend != Backend::OpenGL) {
    reportDeviceCreationFailure(error, GPUErrorCode::InvalidArgument,
                                "descriptor does not select the OpenGL backend");
    return nullptr;
  }
  if (!desc.surface.nativeHandle) {
    reportDeviceCreationFailure(error, GPUErrorCode::InvalidArgument,
                                "the OpenGL backend requires a surface handle");
    return nullptr;
  }
  PortableGLDevice *device = new PortableGLDevice(desc);
  if (!device->initialize()) {
    delete device;
    reportDeviceCreationFailure(
        error, GPUErrorCode::DeviceCreationFailed,
        "the OpenGL context could not be made current or does not meet the "
        "minimum version");
    return nullptr;
  }
